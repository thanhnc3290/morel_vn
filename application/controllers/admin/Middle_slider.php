<?php 
Class Middle_slider extends MY_Controller
{
	function __construct()
	{
        ...
        $this->load->model('middle_slider_model');
        ...
        $list  = $this->middle_slider_model->get_list($input_slider);
        $this->data['list'] = $list;
	...
	*/
	function index()
	{
		...
		$this->data['temp'] ='admin/middle_slider/index';
	...
	function create_modal(){
		...
		if($this->input->post())
		{
			...
			if($this->form_validation->run())
			{
                ...
                $upload_path = './upload/middle_slider';
                ...
                if($this->middle_slider_model->create($data))
				{
				...
				redirect(admin_url('middle_slider')); 
	...
	function edit_modal(){
		...
		if($this->input->post())
		{
			...
			if($this->form_validation->run())
			{
				...
				$id   					= intval($id);
				$info 					= $this->middle_slider_model->get_info($id);
				if(isset($info)){
					// Thêm vào cơ sở dữ liệu
					$name             		= $this->input->post('name');
	                $content             	= $this->input->post('content');
					...
					if($image_link != '')
					{
						$image_link_of_this_info = './upload/middle_slider/'.$info->image_link; // Tạo vị trí ảnh đại diện cũ -- Tương tự với image_list thì foreach nó ra -- để xóa
					...
					if($this->middle_slider_model->update($id, $data))
					{
						//Tạo ra nội dung thông báo
						$this->session->set_flashdata('message','Cập nhật dữ liệu thành công');
	...
	*/
	function delete()
	{
		...
		$this->middle_slider_model->delete($id); // Xóa dữ liệu trong CSDL
